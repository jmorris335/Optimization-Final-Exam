...
extern "C" {
    double cDensity(double z, double &P, double &T) {return Density(z, P, T);}
    double cWaveDrag(double v, double T) {return WaveDrag(v, T);}
    double cThrust(double t, dVector<double> &RocketDefinition, dMatrix<double> &RocketData) {
        return Thrust(t, RocketDefinition, RocketData);
    }
    double cThrottleCalc(double t, dVector<double>& RocketDefinition) {return ThrottleCalc(t, RocketDefinition);}
    double cGrossVehicleMass(dVector<double> &RocketDefinition, dMatrix<double> &RocketData) {
        return GrossVehicleMass(RocketDefinition, RocketData);
    }
    double cFdrag(double rho, double CD, double v, dVector<double> &RocketDefinition, dMatrix<double> &RocketData) {
        return Fdrag(rho, CD, v, RocketDefinition, RocketData);
    }
    double cGravity(double z) {return Gravity(z);}
    double cMassChange(double t, dVector<double>& RocketDefinition, dMatrix<double>& RocketData) {
        return MassChange(t, RocketDefinition, RocketData);
    }
}
