...
def checkOrbitalVelocity(r: Rocket):
    if r.orbital_velocity[0] < MIN_ORBITAL_VEL:
        ...
        max_engines = maxEngines(stage=r.num_stages-1, engine_id=getTypeEngine(r, max(r.num_stages-1,1)))
        if getNumEngines(r, max(r.num_stages-1,1)) >= max_engines:
            addStage(r)
...
def getInitalRocket(engine_id_S1: int=144, engine_id_S2plus: int=213, booster_type: int=22, 
    ...
    r = Rocket(num_stages=num_stages, payload=25000,
               ...
               enginetype_S1=engine_id_S1, num_engines_S1=maxEngines(engine_id=engine_id_S1, failure_threshold=0.98),
               enginetype_S2=engine_id_S2plus, num_engines_S2=maxEngines(engine_id=engine_id_S2plus, failure_threshold=0.98),
               enginetype_S3=engine_id_S2plus, num_engines_S3=maxEngines(engine_id=engine_id_S2plus, failure_threshold=0.98),
               enginetype_S4=engine_id_S2plus, num_engines_S4=maxEngines(engine_id=engine_id_S2plus, failure_threshold=0.98),
               enginetype_S5=engine_id_S2plus, num_engines_S5=maxEngines(engine_id=engine_id_S2plus, failure_threshold=0.98), 
               )
